...
class CreateEventVC: UIViewController {
    ...
    private lazy var emojiAndColorCollectionView: UICollectionView = {
        ...
        collectionView.register(EmojiAndColorSupView.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: EmojiAndColorSupView.identifier)
    ...
    @objc private func categoryButtonAction() {
        let categoryViewModel = CategoryViewModel(delegate: self, selectedCategory: category)
...
extension CreateEventVC: UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(
        ...
        guard let view = emojiAndColorCollectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: id, for: indexPath) as? EmojiAndColorSupView else { return UICollectionReusableView() }
        let section = indexPath.section
        if section == 0 {
